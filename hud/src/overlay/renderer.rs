...
impl D2DRenderer {
    ...
    pub fn render(
        ...
        unsafe {
            ...
            let panel_bg = self.brush(color(0.0, 0.0, 0.0, opacity.clamp(0.1, 1.0)))?;
            ...
            if let Some(ref voice) = state.voice {
                ...
                for member in &voice.members {
                    ...
                    self.draw_member_row(member, is_streaming, &mut y, width as f32)?;
                    y += GAP;
                }
            }
    ...
    unsafe fn draw_header(
        ...
        // Crew avatar (rounded rect) — bitmap if available, initials fallback
        let av_y = *y + (HEADER_HEIGHT - CREW_AVATAR_SIZE) / 2.0;
        let av_radius = 5.0;
        let drew_bitmap = if let Some(ref rgba) = crew.avatar_rgba {
            self.draw_avatar_bitmap(rgba, x, av_y, CREW_AVATAR_SIZE, av_radius)?
        } else {
            false
        };
        if !drew_bitmap {
            let accent = self.brush(color(0.922, 0.302, 0.373, 1.0))?;
            let accent_bg = self.brush(color(0.922, 0.302, 0.373, 0.2))?;
            let av_rr = rounded(x, av_y, CREW_AVATAR_SIZE, CREW_AVATAR_SIZE, av_radius);
            self.rt.FillRoundedRectangle(&av_rr, &accent_bg);
            self.rt.DrawRoundedRectangle(&av_rr, &accent, 1.0, None);
    ...
    unsafe fn draw_member_row(
        ...
        let drew_bitmap = if let Some(ref rgba) = member.avatar_rgba {
            self.draw_avatar_bitmap(rgba, av_x, av_y, AVATAR_SIZE, av_r)?
        } else {
            false
        };
        ...
        if !drew_bitmap {
            ...
            let border_brush = self.brush(av_border)?;
            let av_rect = rounded(av_x, av_y, AVATAR_SIZE, AVATAR_SIZE, av_r);
            self.rt
                .DrawRoundedRectangle(&av_rect, &border_brush, 1.5, None);
        }
        ...
        // Right-side indicator: LIVE badge, speaking bars, or muted icon
        if is_streaming {
            self.draw_live_badge(row_y, ROW_HEIGHT)?;
        } else if member.speaking {
    ...
    unsafe fn draw_live_badge(&self, row_y: f32, row_h: f32) -> Result<()> {
        let red = color(0.922, 0.302, 0.373, 1.0);
        let red_brush = self.brush(red)?;
        ...
        self.rt
            .DrawRoundedRectangle(&badge_rr, &red_brush, 1.0, None);
        ...
        self.rt.FillEllipse(&dot, &red_brush);
    ...
    unsafe fn draw_avatar_bitmap(
        ...
        // Convert RGBA to BGRA (premultiplied) for D2D
        let mut bgra = rgba;
        ...
        let d2d_size = windows::Win32::Graphics::Direct2D::Common::D2D_SIZE_U {
            width: w,
            height: h,
        };
        let bmp = self
            .rt
            .CreateBitmap(d2d_size, Some(bgra.as_ptr() as _), w * 4, &bmp_props)?;

        // Clip to rounded rect using a layer
        ...
        self.rt.PushLayer(&layer_params, None);
