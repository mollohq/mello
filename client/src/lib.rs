...
/// Process entry point, called by the thin `mello` binary in `main.rs`.
///
/// Lives in the library rather than the binary so integration tests and the
...
fn run_app() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Apply saved audio device and processing settings
    {
        ...
        let _ = cmd_tx.send(Command::SetAgc { enabled: s.agc });
...
mod tests {
    ...
    ///
    /// `ElementHandle` queries need element metadata embedded in the generated
    /// code. Without it, every query returns an *empty iterator* rather than an
    /// error — so element-based UI tests would all pass while asserting nothing.
    /// That silent-success mode is the single worst failure for a test harness,
    /// so assert the capability directly.
    #[test]
    fn slint_debug_info_is_available_for_element_queries() {
        init_test_backend();
        let app = MainWindow::new().unwrap();

        // Put the window into a state that actually renders something. Hidden
        // subtrees are absent from element queries, and MainWindow's *default*
        // state (onboarding-step 0, logged-in false) matches neither the
        // onboarding branch (step 1..=3) nor the main app branch — it is the
        // blank window. Querying the default state would prove nothing.
        app.set_onboarding_step(1);

        let elements = i_slint_backend_testing::ElementQuery::from_root(&app)
            .match_descendants()
            .find_all();

        assert!(
            !elements.is_empty(),
            "no elements found in MainWindow: debug info is not in effect, so every \
             ElementHandle query would silently return nothing. Check that \
             CompilerConfiguration::with_debug_info(true) is applied in client/build.rs \
             for this profile."
        );
        assert!(
            elements.iter().any(|e| e.type_name().is_some()),
            "elements found but none expose a type name — debug info is incomplete"
        );
    }
