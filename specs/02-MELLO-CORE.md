...
- **Game audio (Windows):** Host loopback capture → Opus on the stream peer sendonly audio m-line. Viewer recvonly audio uses `RtcpReceivingSession` + RTP `onMessage` (not the voice `onTrack` path). SFU: `AudioTrackData` → `mello_stream_feed_audio_packet`; P2P: `stream_audio_track_callback` → same feed.
