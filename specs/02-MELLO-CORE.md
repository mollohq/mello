...
- **Game audio:** Host system-audio capture (WASAPI loopback on Windows, ScreenCaptureKit on macOS) → Opus on the stream peer sendonly audio m-line. Viewer recvonly audio uses `RtcpReceivingSession` + RTP `onMessage` (not the voice `onTrack` path). SFU: `AudioTrackData` → `mello_stream_feed_audio_packet`; P2P: `stream_audio_track_callback` → same feed.
...
- **In-tick SFU health check (`voice/mod.rs`).** The voice tick sends a control-channel ping (~2s) and evaluates multi-signal health: PeerConnection connected, control channel open, and fresh pong (`pong_age_ms <= 8s`, with `-1` tolerated during initial warmup). During active signaling churn, a short grace window (`signaling_idle_ms <= 5s`) suppresses false negatives. After 3 consecutive unhealthy checks (~6s), it tears down via `mark_disconnected_with_reason("liveness_timeout")` and reconnects. SFU `error`/`session_ended` signaling is surfaced as `SfuEvent::Disconnected`.
